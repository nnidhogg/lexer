...
Dfa::Dfa(const std::size_t init_state, Transitions_t transitions, Accept_states_t accept_states)
    : init_state_{init_state}, transitions_{std::move(transitions)}, accept_states_{std::move(accept_states)}
{}

std::size_t Dfa::init_state() const
{
    return init_state_;
}
...
std::optional<Token> Dfa::find_token(const Dfa& dfa, const State_t state)
{
