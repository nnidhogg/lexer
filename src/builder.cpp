...
{
    const auto dfa{subset_construction(nfa_.build())};

    return Lexer{dfa::Dfa::minimize(dfa)};
}

nfa::Builder Builder::nfa() const
{
    return nfa_;
}
...
dfa::Dfa Builder::subset_construction(const nfa::Nfa& nfa)
{
    ...
    while (!nfa_queue.empty())
    {
        ...
        if (auto token = nfa::Nfa::find_token(nfa, nfa_states); token)
        {
        ...
        std::ranges::for_each(view, [&](const auto symbol) {
            ...
            if (!next_states.empty() && !nfa_dfa_map.contains(next_states))
            {
                nfa_dfa_map.emplace(next_states, dfa.step_state());
