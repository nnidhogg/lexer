...
TEST_F(Concat_test, Two_characters)
{
    ...
    EXPECT_EQ(Simulation::run(nfa, "ab"), Result_t(token, 2));
    EXPECT_EQ(Simulation::run(nfa, "abc"), Result_t(token, 2));
...
TEST_F(Concat_test, Multiple_characters)
{
    ...
    EXPECT_EQ(Simulation::run(nfa, "abcd"), Result_t(token, 4));
    EXPECT_EQ(Simulation::run(nfa, "abcde"), Result_t(token, 4));
    EXPECT_EQ(Simulation::run(nfa, "abcd!"), Result_t(token, 4));
