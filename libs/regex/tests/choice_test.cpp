...
TEST_F(Choice_test, Single_character)
{
    ...
    EXPECT_EQ(Simulation::run(nfa, "b"), Result_t(token, 1));
    EXPECT_EQ(Simulation::run(nfa, "ab"), Result_t(token, 1));

    EXPECT_EQ(Simulation::run(nfa, ""), Result_t(std::nullopt, 0));
    EXPECT_EQ(Simulation::run(nfa, "c"), Result_t(std::nullopt, 0));
}
...
TEST_F(Choice_test, Multiple_characters)
{
    ...
    EXPECT_EQ(Simulation::run(nfa, "a"), Result_t(token, 1));
    EXPECT_EQ(Simulation::run(nfa, "bc"), Result_t(token, 2));
    EXPECT_EQ(Simulation::run(nfa, "def"), Result_t(token, 3));

    EXPECT_EQ(Simulation::run(nfa, ""), Result_t(std::nullopt, 0));
    EXPECT_EQ(Simulation::run(nfa, "b"), Result_t(std::nullopt, 0));
    EXPECT_EQ(Simulation::run(nfa, "de"), Result_t(std::nullopt, 0));
}
