...
TEST_F(Text_test, Simple_text)
{
    ...
    EXPECT_EQ(Simulation::run(nfa, "hello"), Result_t(token, 5));
    EXPECT_EQ(Simulation::run(nfa, "hello!"), Result_t(token, 5));

    EXPECT_EQ(Simulation::run(nfa, ""), Result_t(std::nullopt, 0));
    EXPECT_EQ(Simulation::run(nfa, "hell"), Result_t(std::nullopt, 0));
}
...
TEST_F(Text_test, Special_characters)
{
    ...
    EXPECT_EQ(Simulation::run(nfa, "a*b+c?"), Result_t(token, 6));
...
TEST_F(Text_test, More_special_characters)
{
    ...
    EXPECT_EQ(Simulation::run(nfa, ".*+?^${}()|[]\\"), Result_t(token, 14));
...
TEST_F(Text_test, Empty_text)
{
    ...
    EXPECT_EQ(Simulation::run(nfa, " "), Result_t(token, 0));
    EXPECT_EQ(Simulation::run(nfa, "a"), Result_t(token, 0));
}
