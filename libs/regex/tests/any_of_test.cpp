...
TEST_F(Any_of_test, Single_char)
{
    ...
    {
        EXPECT_EQ(Simulation::run(nfa, std::string{symbol}), Result_t(std::nullopt, 0));
    }

    EXPECT_EQ(Simulation::run(nfa, "ab"), Result_t(token, 1));
    EXPECT_EQ(Simulation::run(nfa, "abc"), Result_t(token, 1));
    EXPECT_EQ(Simulation::run(nfa, "a.b.c"), Result_t(token, 1));

    EXPECT_EQ(Simulation::run(nfa, ""), Result_t(std::nullopt, 0));
    EXPECT_EQ(Simulation::run(nfa, "ba"), Result_t(std::nullopt, 0));
    EXPECT_EQ(Simulation::run(nfa, "cba"), Result_t(std::nullopt, 0));
    EXPECT_EQ(Simulation::run(nfa, "bac"), Result_t(std::nullopt, 0));
    EXPECT_EQ(Simulation::run(nfa, "123"), Result_t(std::nullopt, 0));
    EXPECT_EQ(Simulation::run(nfa, "*[=]"), Result_t(std::nullopt, 0));
}
...
TEST_F(Any_of_test, Multiple_chars)
{
    ...
    EXPECT_EQ(builder.next_state(), 2);
...
TEST_F(Any_of_test, Simulate_multiple_char)
{
    ...
    {
        EXPECT_EQ(Simulation::run(nfa, std::string{symbol}), Result_t(token, 1));
    }
    ...
    EXPECT_EQ(Simulation::run(nfa, "ab"), Result_t(token, 1));
    EXPECT_EQ(Simulation::run(nfa, "ba"), Result_t(token, 1));
    EXPECT_EQ(Simulation::run(nfa, "abc"), Result_t(token, 1));
    EXPECT_EQ(Simulation::run(nfa, "cba"), Result_t(token, 1));
    EXPECT_EQ(Simulation::run(nfa, "bac"), Result_t(token, 1));
    EXPECT_EQ(Simulation::run(nfa, "a.b.c"), Result_t(token, 1));
...
TEST_F(Any_of_test, Simulate_digit_chars)
{
    ...
    EXPECT_EQ(Simulation::run(nfa, "123"), Result_t(token, 1));

    EXPECT_EQ(Simulation::run(nfa, ""), Result_t(std::nullopt, 0));
    EXPECT_EQ(Simulation::run(nfa, "ab"), Result_t(std::nullopt, 0));
    EXPECT_EQ(Simulation::run(nfa, "ba"), Result_t(std::nullopt, 0));
    EXPECT_EQ(Simulation::run(nfa, "abc"), Result_t(std::nullopt, 0));
    EXPECT_EQ(Simulation::run(nfa, "cba"), Result_t(std::nullopt, 0));
    EXPECT_EQ(Simulation::run(nfa, "bac"), Result_t(std::nullopt, 0));
    EXPECT_EQ(Simulation::run(nfa, "*[=]"), Result_t(std::nullopt, 0));
    EXPECT_EQ(Simulation::run(nfa, "a.b.c"), Result_t(std::nullopt, 0));
}
...
TEST_F(Any_of_test, Simulate_printable_chars)
{
    ...
    EXPECT_EQ(Simulation::run(nfa, "bac"), Result_t(token, 1));
    EXPECT_EQ(Simulation::run(nfa, "123"), Result_t(token, 1));
    EXPECT_EQ(Simulation::run(nfa, "*[=]"), Result_t(token, 1));
    EXPECT_EQ(Simulation::run(nfa, "a.b.c"), Result_t(token, 1));
