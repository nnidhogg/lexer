...
[[nodiscard]] nfa::Builder Repeat::to_kleene() const
{
    ...
    std::ranges::for_each(S.accept_states(), [&S](const auto& pair) {
        S.add_epsilon_transition(pair.first, S.init_state());
    });
...
[[nodiscard]] nfa::Builder Repeat::to_at_least(const std::size_t min) const
{
    ...
    std::ranges::for_each(std::views::iota(static_cast<std::size_t>(1), min), [this, &S](auto) {
        S = S.append(regex_->to_nfa());
    });
...
[[nodiscard]] nfa::Builder Repeat::to_range(const std::size_t min, const std::size_t max) const
{
    ...
    std::ranges::for_each(std::views::iota(static_cast<std::size_t>(0), min), [this, &S](auto) {
        S = S.append(regex_->to_nfa());
    });
