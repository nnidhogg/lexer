...
Nfa::Nfa(const std::size_t init_state, Transitions_t transitions, Accept_states_t accept_states)
    : init_state_{init_state}, transitions_{std::move(transitions)}, accept_states_{std::move(accept_states)}
{}

std::size_t Nfa::init_state() const
{
    return init_state_;
}

const Nfa::Transitions_t& Nfa::transitions() const
{
    return transitions_;
}

const Nfa::Accept_states_t& Nfa::accept_states() const
{
...
std::optional<Token> Nfa::find_token(const Nfa& nfa, const States_t& states)
{
