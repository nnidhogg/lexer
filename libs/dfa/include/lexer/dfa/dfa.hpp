...
public:
    ...
    using Accept_states_t = std::unordered_map<std::size_t, Token>;

    Dfa(std::size_t init_state, Transitions_t transitions, Accept_states_t accept_states);
    ...
    [[nodiscard]] static Dfa minimize(const Dfa& dfa);

    [[nodiscard]] static std::optional<State_t> advance(const Dfa& dfa, State_t state, char symbol);

    [[nodiscard]] static std::optional<Token> find_token(const Dfa& dfa, State_t state);
